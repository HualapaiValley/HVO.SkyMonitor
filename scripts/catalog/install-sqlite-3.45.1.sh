...
if [[ "$fetch" == true ]]; then
    curl --fail --location --proto '=https' --tlsv1.2 --connect-timeout 30 --max-time 180 \
        --max-redirs 5 --max-filesize "$SQLITE_SOURCE_LENGTH" "$SQLITE_SOURCE_URL" --output "$archive"
else
...
if [[ -e "$INSTALL_ROOT" || -L "$INSTALL_ROOT" ]]; then
    [[ -d "$INSTALL_ROOT" && ! -L "$INSTALL_ROOT" && -d "$INSTALL_ROOT/bin" && ! -L "$INSTALL_ROOT/bin" && \
       -f "$INSTALL_ROOT/bin/sqlite3" && ! -L "$INSTALL_ROOT/bin/sqlite3" && -x "$INSTALL_ROOT/bin/sqlite3" && \
       -f "$INSTALL_ROOT/source.sha256" && ! -L "$INSTALL_ROOT/source.sha256" ]] || \
    ...
    [[ "$(stat -c '%u' "$INSTALL_ROOT/bin/sqlite3")" == 0 ]] || fail "existing SQLite CLI is not owned by root"
    installed_mode="$(stat -c '%a' "$INSTALL_ROOT/bin/sqlite3")"
    (( (8#$installed_mode & 0022) == 0 )) || fail "existing SQLite CLI is group- or world-writable"
    installed_sha256="$(sha256sum "$INSTALL_ROOT/bin/sqlite3")"
    built_sha256="$(sha256sum "$built")"
    [[ "${installed_sha256%% *}" == "${built_sha256%% *}" ]] || \
      fail "existing SQLite CLI differs from the checksum-verified source build"
...
chmod 0755 "$INSTALL_ROOT" "$INSTALL_ROOT/bin"
