...
public sealed class LogicHostDerivativeWorkerPerformanceTests
{
    ...
    public async Task CentralDerivativeWorker_CanonicalWorkloads_RecordPerformanceEvidence()
    {
        ...
        var evidence = new
        {
            ...
            Command = "dotnet test tests/HVO.SkyMonitor.LogicHost.IntegrationTests/HVO.SkyMonitor.LogicHost.IntegrationTests.csproj --configuration Release --filter \"FullyQualifiedName~LogicHostDerivativeWorkerPerformanceTests\"",
            Scale = new
            {
    ...
    public async Task CentralDerivativeWorker_P5C1FreshProcess_RecordsDiagnosticEvidence()
    {
        ...
        await File.WriteAllTextAsync(
            ...
            JsonSerializer.Serialize(new
            {
                ...
                Trial = trial,
                Command = $"DOTNET_gcServer=1 {P5DiagnosticTrialVariable}={trial} HVO_EVIDENCE_REVISION={revision} dotnet test tests/HVO.SkyMonitor.LogicHost.IntegrationTests/HVO.SkyMonitor.LogicHost.IntegrationTests.csproj --no-build --configuration Release --filter FullyQualifiedName~LogicHostDerivativeWorkerPerformanceTests.CentralDerivativeWorker_P5C1FreshProcess_RecordsDiagnosticEvidence",
