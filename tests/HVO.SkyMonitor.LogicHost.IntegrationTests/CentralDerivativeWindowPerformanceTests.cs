...
public sealed class CentralDerivativeWindowPerformanceTests
{
    ...
    public async Task Phase10P1ToP4_RecordsCanonicalEvidence()
    {
        ...
        var evidence = new
        {
            ...
            RecordedAtUtc = DateTimeOffset.UtcNow,
            Command = "dotnet test tests/HVO.SkyMonitor.LogicHost.IntegrationTests/HVO.SkyMonitor.LogicHost.IntegrationTests.csproj --no-build --configuration Release --filter \"FullyQualifiedName~CentralDerivativeWindowPerformanceTests\"",
            Environment = new
            {
    ...
    public async Task Issue116CentralTransientW2W3MW4_RecordsAcceptanceEvidence()
    {
        ...
        var evidence = new
        {
            ...
            RecordedAtUtc = DateTimeOffset.UtcNow,
            Command = "dotnet test tests/HVO.SkyMonitor.LogicHost.IntegrationTests/HVO.SkyMonitor.LogicHost.IntegrationTests.csproj --configuration Release --arch x64 --filter FullyQualifiedName~CentralDerivativeWindowPerformanceTests.Issue116CentralTransientW2W3MW4_RecordsAcceptanceEvidence",
            Environment = new
            {
                Framework = RuntimeInformation.FrameworkDescription,
                OperatingSystem = RuntimeInformation.OSDescription,
