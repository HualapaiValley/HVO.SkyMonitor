...
public sealed class ProtectedApiTests
{
    ...
    public async Task ProtectedStatusWithBearerTokenSucceedsAsync()
    {
        ...
        var user = payload.EnumerateObject().Single(property =>
            property.Name.Equals("user", StringComparison.OrdinalIgnoreCase)).Value;
        var authenticationType = user.EnumerateObject().Single(property =>
            property.Name.Equals("authenticationType", StringComparison.OrdinalIgnoreCase)).Value.GetString();
        Assert.AreEqual(CentralArtifactCredentialAccess.BearerAuthenticationType, authenticationType);
    }
    ...
    [TestMethod]
    public async Task ProtectedStatusWithBearerAndApiKeyIsForbiddenAsync()
    {
        var token = await HttpHelpers.GetPasswordTokenAsync(
            _client!,
            "/connect/token",
            ...
            TestUsers.Operator.Password,
            TestClients.WebUI.ClientId,
            "openid profile").ConfigureAwait(false);
        using var request = new HttpRequestMessage(
            HttpMethod.Get,
            new Uri("/api/v1.0/status/protected", UriKind.Relative));
        request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue(
            "Bearer",
            token.AccessToken);
        request.Headers.Add(ApiKeyAuthenticationOptions.HeaderName, TestApiKeys.InternalService.Key);
    ...
    [TestMethod]
    public async Task LogicHostCookieFactoryEmitsCanonicalUserIdentityAsync()
    {
        await using var scope = AssemblyHooks.Fixture.Factory.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var user = await db.Users.SingleAsync(item => item.Email == TestUsers.Operator.Email).ConfigureAwait(false);
        var factory = scope.ServiceProvider.GetRequiredService<IUserClaimsPrincipalFactory<ApplicationUser>>();

        var principal = await factory.CreateAsync(user).ConfigureAwait(false);

        var identity = CentralArtifactCredentialAccess.GetSingleCredentialIdentity(principal);
        Assert.IsNotNull(identity);
        Assert.AreEqual(IdentityConstants.ApplicationScheme, identity.AuthenticationType);
        Assert.AreEqual(user.Id, CentralArtifactCredentialAccess.GetOwnerId(principal));
        Assert.AreEqual("User", identity.FindFirst("account_type")?.Value);
    }

    [TestMethod]
    public async Task BearerScopesFailClosedForReadAndWriteAsync()
    {
        var client = _client!;
        var noApiScope = await HttpHelpers.GetPasswordTokenAsync(
            client,
            "/connect/token",
            TestUsers.Operator.Username,
            TestUsers.Operator.Password,
        ...
        {
            readRequest.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue(
                "Bearer",
                noApiScope.AccessToken);
            using var readResponse = await client.SendAsync(readRequest).ConfigureAwait(false);
            Assert.AreEqual(HttpStatusCode.Forbidden, readResponse.StatusCode);
        }

        var readOnly = await HttpHelpers.GetPasswordTokenAsync(
            client,
            "/connect/token",
            TestUsers.Operator.Username,
            TestUsers.Operator.Password,
        ...
        using var writeRequest = new HttpRequestMessage(
            ...
            new Uri("/api/internal/observatories", UriKind.Relative))
        {
            Content = JsonContent.Create(new { name = "Insufficient scope" })
        };
        writeRequest.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue(
            "Bearer",
            readOnly.AccessToken);
        using var writeResponse = await client.SendAsync(writeRequest).ConfigureAwait(false);
        Assert.AreEqual(HttpStatusCode.Forbidden, writeResponse.StatusCode);
    }
    ...
    [TestMethod]
    public async Task ObservatoryReadWithMergedBearerAndApiKeyIdentitiesIsForbiddenAsync()
    {
        await using var scope = AssemblyHooks.Fixture.Factory.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var owner = await db.Users.SingleAsync(user => user.Email == TestUsers.Operator.Email).ConfigureAwait(false);
    ...
    private static CentralArtifact CreateScopedArtifact(Guid observatoryId, string suffix)
    {
        ...
        var artifact = new CentralArtifact
        {
            ...
            StorageReference = $"object://scope-test/{suffix}",
