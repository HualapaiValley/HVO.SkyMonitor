...
public sealed class RunnerHostTests
{
    ...
    [TestMethod]
    [TestCategory("Unit")]
    public async Task AStopFilePresentBeforeClaimingStartsRetiresTheHostWithoutAnyClaim()
    {
        using var handler = new ScriptedHttpHandler();
        var prefix = $"/{ProcessingRunnerProtocol.RoutePrefix}/{RunnerTestData.RunnerId}";
        handler.MapJson(HttpMethod.Put, prefix, _ => RunnerTestData.Registration());
        ...
        using var output = new StringWriter();
        var stopFile = Path.Combine(Path.GetTempPath(), $"hvo-runner-stop-{Guid.NewGuid():N}");
        // The retirement was requested while the runner was still registering: the stop file exists before claiming starts.
        await File.WriteAllTextAsync(stopFile, "stop");
        using var host = new RunnerHost(
            client,
            new ProcessingRecipeExecutor(),
        ...
        {
            using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(30));
            Assert.AreEqual(0, await host.RunAsync(timeout.Token));
            var methods = handler.Requests.Select(item => $"{item.Method} {item.Path}").ToArray();
            Assert.IsFalse(methods.Contains($"POST {prefix}/claims"), "no claim slot opens when the stop file is already present");
            Assert.AreEqual($"DELETE {prefix}", methods[^1], "the host retires directly");
            StringAssert.Contains(output.ToString(), "\"event\":\"stop-requested\"", StringComparison.Ordinal);
        }
        finally
        {
    ...
    public async Task HeartbeatCancellationStopsExecutionWithoutCompletingTheJob()
    {
        ...
        handler.Map(HttpMethod.Get, claim.Inputs[0].ContentPath, (_, _) => ScriptedHttpHandler.Bytes(RunnerTestData.NoOpPayload));
        handler.MapJson(HttpMethod.Post, $"{prefix}/heartbeat",
            _ => new ProcessingRunnerHeartbeatResponse(ProcessingRunnerRegistrationStatus.Active, [claim.JobId], [], [BuiltInProcessingRecipes.NoOpAnalyzer], DateTimeOffset.UtcNow));
        handler.Map(HttpMethod.Delete, prefix, (_, _) => new HttpResponseMessage(HttpStatusCode.NoContent));
        using var http = RunnerTestData.CreateHttpClient(handler);
        using var client = new ProcessingRunnerClient(http, RunnerTestData.ClientOptions());
        using var output = new StringWriter();
    ...
    [TestMethod]
    [TestCategory("Unit")]
    public async Task ShutdownStopsClaimingAndLetsActiveJobsFinishWithinTheGracePeriod()
    {
        var claim = RunnerTestData.NoOpClaim();
        using var handler = new ScriptedHttpHandler();
        var prefix = $"/{ProcessingRunnerProtocol.RoutePrefix}/{RunnerTestData.RunnerId}";
        ...
        using var client = new ProcessingRunnerClient(http, RunnerTestData.ClientOptions());
        using var output = new StringWriter();
        var executor = new SlowExecutor(TimeSpan.FromSeconds(2));
        using var host = new RunnerHost(
            client,
            executor,
            new RunnerHostOptions(
                RunnerTestData.RunnerId, "Test", RunnerTestData.Capabilities(), 1,
                ProcessingRunnerProtocol.MaximumTransferBytes, TimeSpan.Zero, TimeSpan.FromSeconds(10), TimeSpan.FromMilliseconds(50)),
            new RunnerLog(output, output, RunnerTestData.RunnerId));

        using var shutdown = new CancellationTokenSource();
        var run = host.RunAsync(shutdown.Token);
        await executor.Started.Task.WaitAsync(TimeSpan.FromSeconds(20));
        await shutdown.CancelAsync();
        var exit = await run.WaitAsync(TimeSpan.FromSeconds(30));

        Assert.AreEqual(0, exit);
        Assert.IsFalse(executor.WasCanceled, "the in-flight recipe must finish inside the grace period");
        Assert.IsTrue(handler.Requests.Any(item => item.Path.EndsWith("/completion", StringComparison.Ordinal)));
        StringAssert.Contains(output.ToString(), "\"event\":\"draining\"", StringComparison.Ordinal);
        StringAssert.Contains(output.ToString(), "\"event\":\"job-completed\"", StringComparison.Ordinal);
    }

    [TestMethod]
    [TestCategory("Unit")]
    public async Task HeartbeatRefreshesEligibilityWithoutReRegistration()
    {
        var claim = RunnerTestData.NoOpClaim();
        using var handler = new ScriptedHttpHandler();
        var prefix = $"/{ProcessingRunnerProtocol.RoutePrefix}/{RunnerTestData.RunnerId}";
        var claimed = 0;
        handler.MapJson(HttpMethod.Put, prefix, _ => RunnerTestData.Registration(heartbeat: TimeSpan.FromMilliseconds(100), eligible: []));
        handler.Map(HttpMethod.Post, $"{prefix}/claims", (_, _) => Interlocked.Increment(ref claimed) == 1
            ? ScriptedHttpHandler.Json(claim)
            : new HttpResponseMessage(HttpStatusCode.NoContent));
        handler.Map(HttpMethod.Get, claim.Inputs[0].ContentPath, (_, _) => ScriptedHttpHandler.Bytes(RunnerTestData.NoOpPayload));
        ...
        Assert.AreEqual(0, exit);
        Assert.AreEqual(1, handler.Requests.Count(item => item.Method == "PUT"), "no re-registration was needed");
        Assert.IsTrue(handler.Requests.Any(item => item.Path.EndsWith("/completion", StringComparison.Ordinal)));
        StringAssert.Contains(output.ToString(), "\"event\":\"eligibility-changed\"", StringComparison.Ordinal);
    }

    private sealed class SlowExecutor(TimeSpan duration) : IProcessingRecipeExecutor
    {
        ...
        public async ValueTask<ProcessingOutcome> ExecuteAsync(ProcessingExecutionRequest request, CancellationToken cancellationToken)
        {
            ...
            return await new ProcessingRecipeExecutor().ExecuteAsync(request, cancellationToken);
        }
    }
