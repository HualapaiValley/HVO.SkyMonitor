...
public sealed partial class CentralArtifactRetentionPerformanceTests
{
    ...
    public async Task RuntimeSqlPolicy_W4AndDelayedObjectIo_RecordsEvidence()
    {
        ...
        var evidence = new
        {
            ...
            Command = $"{Issue254OptInVariable}=1 DOTNET_gcServer=1 HVO_EVIDENCE_PHASE=baseline|after HVO_EVIDENCE_REVISION=<HARNESS_OR_CANDIDATE_HEAD> HVO_EVIDENCE_PRODUCTION_REVISION=<PRODUCTION_HEAD> HVO_EVIDENCE_TRIAL=1..5 dotnet test tests/HVO.SkyMonitor.IntegrationTests/HVO.SkyMonitor.IntegrationTests.csproj --no-build --configuration Release --filter FullyQualifiedName~CentralArtifactRetentionPerformanceTests.RuntimeSqlPolicy_W4AndDelayedObjectIo_RecordsEvidence",
        ...
        EvidenceSourceSnapshot source)
    {
        ...
        if (phase == "baseline")
        {
            ...
            var permitted = new HashSet<string>(StringComparer.Ordinal)
            {
                ...
                "tests/HVO.SkyMonitor.IntegrationTests/CentralArtifactRetentionIssue254PerformanceTests.cs",
                "tests/HVO.SkyMonitor.IntegrationTests/CentralArtifactRetentionPerformanceTests.cs"
            };
    ...
    private static string ComputeIssue254HarnessSha256(string repositoryRoot)
    {
        var paths = new[]
        {
            "tests/HVO.SkyMonitor.IntegrationTests/CentralArtifactRetentionIssue254PerformanceTests.cs",
            "tests/HVO.SkyMonitor.IntegrationTests/CentralArtifactRetentionPerformanceTests.cs",
            "tests/HVO.SkyMonitor.IntegrationTests/Infrastructure/Issue246RetentionEvidenceCollector.cs",
