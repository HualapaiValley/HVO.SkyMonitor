...
**Design authority.** Epic #987 adopts this prototype, unchanged from
`origin/prototype/pipeline-operations-preview` at `b5cdec4f`, as the agreed
design for the CameraAgent operator UI. It supersedes the #440 checkpoint's
rejection of the calendar, events, time-lapse, keogram and product-scheduling
concepts and #514's "remain unauthorized" clause for them. Sections whose
logic did not exist when the prototype was drawn (meteor/transient, cloud,
astrometry, satellites) are implemented as that logic lands; until then their
Blazor counterparts render an honest "not produced" state. The fixture data in
these pages is illustrative and is never copied into production pages.
