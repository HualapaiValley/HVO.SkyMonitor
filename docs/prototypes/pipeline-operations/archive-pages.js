...
function initializeEventDetail() {
    ...
    container.innerHTML = `
        ...
        ${renderSatelliteAssessment(event, false)}
...
function renderGenericEventDetail(container, event) {
    ...
    container.innerHTML = `
        ...
        ${event.classification === "satellite" ? renderSatelliteAssessment(event, true) : ""}
...
function renderSatelliteAssessment(event, strongMatch) {
    const provenance = strongMatch
        ? { solution: "ast-E1019-01 / 0.91 px RMS", snapshot: "orbit-20260820T010000Z-a92c", age: "1h 11m / fresh", assessment: "sat-E1019-02" }
        : { solution: "ast-84220-01 / 0.74 px RMS", snapshot: "orbit-20260901T020000Z-7f31", age: "1h 14m / fresh", assessment: "sat-E1042-01" };
    const matches = strongMatch
        ? [
            { object: "Object 41866", confidence: "0.93", separation: "0.18 deg", timing: "+0.07s", reason: "Direction, speed, and time overlap agree" },
            { object: "Object 48274", confidence: "0.29", separation: "3.84 deg", timing: "-1.31s", reason: "Crossing geometry; timing outside uncertainty" }
        ]
        : [
            { object: "Object 48274", confidence: "0.38", separation: "6.80 deg", timing: "+0.42s", reason: "Direction conflicts with measured angular path" },
            { object: "Object 41866", confidence: "0.21", separation: "9.14 deg", timing: "-2.06s", reason: "Outside spatial and timing uncertainty" }
        ];
    return `
        <section class="satellite-assessment archive-panel" aria-labelledby="satellite-assessment-heading">
            <header><div><p class="eyebrow">Orbital correlation / append-only assessment</p><h2 id="satellite-assessment-heading">Orbital match evidence</h2><p>Measured transient geometry is compared with time-parameterized predicted tracks; ranked candidates remain evidence, not automatic rejection.</p></div><span class="state-chip ${strongMatch ? "warning" : "neutral"}">${strongMatch ? "Likely satellite" : "No strong match"}</span></header>
            <div class="satellite-assessment-layout">
                <div class="satellite-correlation-visual">
                    <svg viewBox="0 0 700 240" role="img" aria-label="Measured event path and predicted satellite tracks"><path class="uncertainty" d="M74 188C210 149 341 110 620 50"></path><path class="measured" d="M78 185C216 145 350 105 617 54"></path><path class="predicted primary" d="${strongMatch ? "M82 190C219 148 354 108 622 56" : "M70 199C203 170 358 125 628 79"}"></path><path class="predicted secondary" d="M115 45C258 83 411 125 586 196"></path></svg>
                    <div class="satellite-correlation-key"><span class="measured">Measured event</span><span class="predicted">Predicted track</span><span class="uncertainty">Uncertainty</span></div>
                    <dl><div><dt>Astrometric solution</dt><dd>${provenance.solution}</dd></div><div><dt>Orbital snapshot</dt><dd>${provenance.snapshot}</dd></div><div><dt>Snapshot age</dt><dd>${provenance.age}</dd></div><div><dt>Optical visibility</dt><dd>Not established by orbital data</dd></div></dl>
                </div>
                <ol class="satellite-match-list">${matches.map((match, index) => `<li><span class="satellite-rank">${index + 1}</span><span><strong>${match.object}</strong><small>${match.reason}</small></span><dl><div><dt>Confidence</dt><dd>${match.confidence}</dd></div><div><dt>Path separation</dt><dd>${match.separation}</dd></div><div><dt>Timing delta</dt><dd>${match.timing}</dd></div></dl></li>`).join("")}</ol>
            </div>
            <footer><span><strong>Assessment ${provenance.assessment}</strong> / producer orbital-correlation-v1 / prior assessments preserved</span><span>${strongMatch ? "Reviewer may retain or revise classification" : `${event.title} remains classified from measured evidence`}</span></footer>
        </section>`;
}
