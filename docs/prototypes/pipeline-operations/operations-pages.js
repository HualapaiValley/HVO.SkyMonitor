...
const pipelineNodeDetails = {
    projected: { title: "Projected scene", type: "Required transform", description: "Projects catalog objects and image geometry directly from the immutable raw frame.", input: "$raw", output: "projected-scene-v2", alias: "projected-scene / ProjectedScene", options: "HYG 4.4 + OpenNGC / exposure-aware" },
...
function renderPipeline() {
    return `
        ...
        <section class="ops-pipeline-workspace" aria-label="Pipeline configuration graph">
            ...
            <aside class="ops-pipeline-inspector" aria-live="polite"><p class="eyebrow" id="pipelineNodeType">Required transform</p><h3 id="pipelineNodeTitle">Projected scene</h3><p id="pipelineNodeDescription">Projects catalog objects and image geometry directly from the immutable raw frame.</p><dl class="ops-node-facts"><div><dt>Step ID / registered type</dt><dd id="pipelineNodeAlias">projected-scene / ProjectedScene</dd></div><div><dt>Input selection</dt><dd id="pipelineNodeInput">$raw</dd></div><div><dt>Outputs</dt><dd id="pipelineNodeOutput">projected-scene-v2</dd></div><div><dt>Effective options</dt><dd id="pipelineNodeOptions">HYG 4.4 + OpenNGC / exposure-aware</dd></div></dl><p class="ops-inspector-footnote">Representative 7-step projection. Calibrated preview, quality, presentation layers, storage, and telemetry remain part of the 14-step effective graph.</p></aside>
        </section>
...
function renderSoftware() {
    return `
        ${pageHeader("System / verified local assets", "Software & catalog", "Inspect application, immutable celestial packages, and separately refreshed orbital evidence without implying unattended activation.", `${prototypeButton("Check availability", "Approved package availability checked; installed bytes and active selection were not changed.")} ${prototypeButton("Verify checksums", "Installed software and catalog checksums verified in prototype.", "primary")}`)}
        <section class="ops-status-rail"><div><span>Selected HYG</span><strong>4.4</strong><small>hyg-v44-production</small></div><div><span>Installed packages</span><strong>2</strong><small>Side by side / verified</small></div><div><span>Catalog availability</span><strong>Current</strong><small>Checked 03:12 UTC</small></div><div><span>Orbital snapshot</span><strong>Fresh</strong><small>Age 1h 14m</small></div></section>
        <div class="ops-note-banner"><span class="status-icon success" aria-hidden="true"></span><span><strong>Selection is explicit.</strong> Availability checks do not download, rebuild, install, or activate a catalog. Published package bytes remain immutable and historical captures retain their exact selected identity.</span></div>
        <div class="ops-grid two"><section class="ops-panel"><header class="ops-panel-heading"><div><h3>CameraAgent software</h3><p>Lifecycle operations remain installer and CLI driven.</p></div><span class="state-chip success">Verified</span></header><dl class="ops-facts"><div><dt>Version</dt><dd>9.0.0-preview.14</dd></div><div><dt>Channel</dt><dd>Local prototype</dd></div><div><dt>Runtime</dt><dd>.NET 10 / linux-x64</dd></div><div><dt>Uptime</dt><dd>12d 4h</dd></div><div class="full"><dt>Image digest</dt><dd><code>sha256:9a1c...e842</code></dd></div><div><dt>Rollback candidate</dt><dd>preview.13</dd></div><div><dt>Update service</dt><dd>Not configured</dd></div></dl></section><section class="ops-panel"><header class="ops-panel-heading"><div><h3>Selected celestial package</h3><p>Read-only local data with exact component provenance.</p></div><span class="state-chip success">Selected</span></header><dl class="ops-facts"><div><dt>Catalog ID</dt><dd>hyg-v44-production</dd></div><div><dt>Package</dt><dd>4.4.0 / official</dd></div><div><dt>Star rows</dt><dd>119,626</dd></div><div><dt>Schema</dt><dd>celestial-snapshot-v3</dd></div><div><dt>Deep sky</dt><dd>OpenNGC 2026.05</dd></div><div><dt>Selection scope</dt><dd>This CameraAgent</dd></div><div class="full"><dt>Package checksum</dt><dd><code>sha256:72c1...4e91</code></dd></div></dl></section></div>
        <section class="ops-panel ops-grid"><header class="ops-panel-heading"><div><h3>Installed celestial packages</h3><p>Rollback changes the selected identity; it never edits or replaces a published SQLite snapshot.</p></div><span>2 verified</span></header><div class="ops-table-wrap"><table class="ops-table"><thead><tr><th>Catalog ID</th><th>Package</th><th>State</th><th>Installed</th><th>Historical use</th></tr></thead><tbody><tr class="current-row"><td><strong>hyg-v44-production</strong></td><td>HYG 4.4 + OpenNGC 2026.05</td><td><span class="state-chip success">Selected</span></td><td>31 Aug 2026</td><td>#84220 onward</td></tr><tr><td><strong>hyg-v42-production</strong></td><td>HYG 4.2 / stars</td><td><span class="state-chip neutral">Retained</span></td><td>19 Jul 2026</td><td>#1-84219</td></tr></tbody></table></div></section>
        <div class="ops-grid sidebar-right"><section class="ops-panel orbital-snapshot-panel"><header class="ops-panel-heading"><div><h3>Orbital-element snapshot</h3><p>Time-varying evidence is refreshed and assessed independently from HYG/OpenNGC packages.</p></div><span class="state-chip success">Fresh</span></header><div class="orbital-freshness"><span><strong>1h 14m old</strong><small>Freshness warning at 24h / expiry policy remains source-qualified</small></span><span class="orbital-freshness-track"><i></i></span><span>Next attempt 05:00 UTC</span></div><dl class="ops-facts"><div class="full"><dt>Snapshot identity</dt><dd><code>orbit-20260901T020000Z-7f31</code></dd></div><div><dt>Objects</dt><dd>11,842</dd></div><div><dt>Last outcome</dt><dd>Validated and activated</dd></div><div><dt>Normalized format</dt><dd>Provider-neutral records</dd></div><div><dt>Scheduler state</dt><dd>Waiting / due 05:00</dd></div><div class="full"><dt>Use boundary</dt><dd>Predicted geometry only; ordinary orbital data does not establish optical visibility.</dd></div></dl></section><section class="ops-panel"><header class="ops-panel-heading"><div><h3>Lifecycle separation</h3><p>Static celestial truth and changing orbital predictions have different safety rules.</p></div></header><div class="ops-table-wrap"><table class="ops-table lifecycle-comparison"><thead><tr><th>Evidence</th><th>Activation</th><th>Freshness</th></tr></thead><tbody><tr><td><strong>HYG / OpenNGC</strong><small>Official package</small></td><td>Explicit selection</td><td>Immutable version</td></tr><tr><td><strong>Orbital elements</strong><small>Durable snapshot</small></td><td>Validated candidate</td><td>Age + stale policy</td></tr><tr><td><strong>Capture binding</strong><small>Provenance</small></td><td>Exact identity</td><td>Never rewritten</td></tr></tbody></table></div><div class="ops-note-banner compact"><span class="status-icon warning" aria-hidden="true"></span><span>Provider and staleness policy shown here are a bounded UI scenario pending the owning orbital-source decision.</span></div></section></div>
        <section class="ops-panel ops-grid"><header class="ops-panel-heading"><div><h3>Lifecycle boundary</h3><p>The static design distinguishes read-only facts from capabilities not yet exposed through Blazor.</p></div></header><div class="ops-table-wrap"><table class="ops-table"><thead><tr><th>Operation</th><th>Current interface</th><th>UI state</th><th>Safety boundary</th></tr></thead><tbody><tr><td><strong>Status, freshness &amp; verification</strong></td><td>UI / CLI</td><td><span class="state-chip success">Available</span></td><td>Read-only installed and scheduler facts</td></tr><tr><td><strong>Catalog select / rollback</strong></td><td>CLI</td><td>Informational</td><td>Atomic validation and explicit selection</td></tr><tr><td><strong>Orbital refresh / quarantine</strong></td><td>Scheduler / CLI</td><td>Informational</td><td>Last-known-good survives candidate failure</td></tr><tr><td><strong>Application upgrade / rollback</strong></td><td>Installer CLI</td><td>Informational</td><td>Candidate health before capture resumes</td></tr><tr><td><strong>Uninstall / purge</strong></td><td>Installer CLI</td><td>Not exposed</td><td>Separate confirmation and retained state</td></tr></tbody></table></div></section>`;
}
