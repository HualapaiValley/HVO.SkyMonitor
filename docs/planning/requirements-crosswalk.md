...
| `ARCH-001`-`ARCH-007` | Scoped roadmap/architecture authority, complete dependency graph, no production TestSupport, no host cross-reference, and explicit AgentCore/Astronomy/Imaging/Processing/Common/catalog/CameraAgent replay/host ownership | `docs/roadmap.md`; `docs/project-plan.md` section 3 and phase 0 | #90, #91, #425 |
...
| `OPS-005`-`OPS-010` | SQL/Redis/provider-neutral object-storage/Mailpit ownership, names/prefixes/logical buckets, safe migration/reset, backup/restore, and reconciliation | `docs/project-plan.md`; `docs/runbooks/*.md` | #111, #114, #120, #243, #108, #499, #504, #584, #592, #585, #586, #506 |
...
| `REPLAY-LOCAL-001`-`REPLAY-LOCAL-010` | Explicit archived-replay-only eligibility, retained in-process topology, authenticated bounded local transport, immutable checksummed inputs and validated outputs, lease/cancellation/deadline fencing, bounded no-fallback backlog behavior, lower-priority resource and scratch isolation, equivalent output identity/provenance, lifecycle observability and diagnostics, and Linux x64/ARM64 evidence | `docs/project-plan.md` section 3.2 | `RM-005`, `RM-017`; #425 and #535; operations in `docs/runbooks/deployment-installer.md`; delivery evidence in PR #542 |
| `CA-PRODUCT-001`-`CA-PRODUCT-008` | Guarded incompatible-state disposition, authenticated shell and account recovery, coherent Current Sky/archive/setup/operations workflows, named-graph and archived-replay operation, signed multi-architecture offline distribution, and final standalone proof with zero required central dependencies | `docs/roadmap.md` `RM-017`; epic #513 and child acceptance criteria | #507, #514, #515, #516, #517, #534, #535, #563, #564 |
| `CA-EXPORT-001`-`CA-EXPORT-008` | Transport-neutral immutable graph-execution evidence, canonical identity and hashing, bounded payloads and sequencing, durable retry and retention, gap detection and resynchronization, exact acknowledgement, deny-sink conformance, and no dependency from local correctness on export | `docs/contracts/cameraagent-execution-evidence-v1.md` (contract and CameraAgent exporter); `docs/runbooks/cameraagent-retention.md` section `Execution Evidence Export`; `docs/planning/performance-validation.md` issue-537 evidence; the `CameraAgent:ExecutionEvidenceExport` options and the owner-only `/api/v1/operations/outboxes/execution-evidence` surface; `docs/roadmap.md` `RM-017`; epic #513 and child acceptance criteria | #536, #537 |
...
| IDs | Requirement group | Owning source | Phase/issues |
| --- | --- | --- | --- |
| `OBJ-001`-`OBJ-002` | Provider-neutral LogicHost object-store operations, logical identity, provider selection/configuration, normalized semantics, authenticated health, integrity, universal conformance, and infrastructure ownership | `docs/project-plan.md` section 3.12; `docs/planning/object-storage-provider-decision.md` | `RM-016`; #499, #504, #584 |
| `OBJ-003` | Host-neutral filesystem durability primitives with separate LogicHost and CameraAgent storage/lifecycle contracts | `docs/project-plan.md` sections 3.2 and 3.12; object-storage decision | `RM-016`; #592; post-`RM-017` adoption #587 |
| `OBJ-004` | Durable LogicHost filesystem provider and exact same-host Linux ext4 qualification covering security, recovery, faults, backup/restore, observability, performance, and native amd64/arm64 | `docs/project-plan.md` section 3.12; object-storage decision | `RM-016`; #585, #586 |
| `OBJ-005` | Fresh-state supported filesystem adoption, least-privilege mount provisioning, deployment/test/CI replacement, operational lifecycle, and removal of current MinIO ownership | `docs/project-plan.md` section 3.12; object-storage decision | `RM-016`; #506 |
| `OBJ-REMOTE-001` | One exact remote filesystem/server/mount profile with proven rename, flush, cache, outage, remount, backup/restore, security, and performance semantics | `docs/project-plan.md` section 3.12; object-storage decision | `RM-019`; #588, #591 |
| `S3-001`-`S3-002` | Retained provider-specific S3 adapter behavior plus independent external S3/LAN or AWS qualification and exact-profile deployment/preflight adoption | `docs/project-plan.md` section 3.12; object-storage decision | Delivered boundary #504; `RM-019` #588, #589; SeaweedFS 4.44 no-go #505 |
| `AZURE-OBJ-001` | Native Azure Blob adapter and independently qualified Azure identity, security, recovery, capacity/cost, observability, and performance profile | `docs/project-plan.md` section 3.12; object-storage decision | `RM-019`; #588, #590 |
| `CENTRAL-001`-`CENTRAL-008` | Canonical manifest-v2 and structured-product ingest, reconstructable normalized records, exact capture-time profile binding, fail-closed current location/profile identity, streamed object access, authorized range retrieval, and no storage credential exposure | `docs/project-plan.md` phase 8 | #98, #99, #466, #504, #506; umbrella #60, #458 |
...
| `E2E-001`-`E2E-007` | Real outbox two-host path through the supported object-store provider, complete feature flow, fault injection, output/state validation, observability review, separate long/external/hardware gates, and current-head database critical-section/access-plan disposition | `docs/project-plan.md` phase 14 | #243, #108, #506 |
