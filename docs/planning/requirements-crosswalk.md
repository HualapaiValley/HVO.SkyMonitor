...
| --- | --- | --- | --- |
| `SYS-001`-`SYS-006` | One camera per CameraAgent process; isolated multi-agent hosts; per-agent configuration, identity, pipeline, history, and telemetry; in-process modules by default; offline capture; one LogicHost for many agents/sites | `docs/project-plan.md` sections 1-3 | All; epic #89 |
| `OWN-EDGE-001`-`OWN-EDGE-010` | Acquisition, setpoint control, raw normalization, local recipes, derivatives, bounded history, offline authenticated operation, outbox, telemetry, and local catalog ownership | `docs/project-plan.md` section 3 | Phases 1-7 and 13 |
| `OWN-CENTRAL-001`-`OWN-CENTRAL-006` | Registration/profile versioning, idempotent ingest, central lifecycle, contextual processing, history/UI, and reprocessing | `docs/project-plan.md` section 3 | Phases 8-14 |
...
| `EXEC-001`-`EXEC-011` | Issue readiness, dependencies, compatibility, performance, validation, runtime review, local gate, PR correction cycle, non-green recovery, and resumable handoff | `docs/planning/agent-execution.md` | Every milestone issue |
...
| `MATRIX-001`-`MATRIX-009` | Required Mono16, RGB24, and Bayer16 fisheye/rectilinear/telescope combinations plus reduced CI and full-resolution evidence | `docs/virtual-camera.md` section Required Compatibility Matrix | #92, #93, #108 |
...
| `ENV-001`-`ENV-003` | Versioned time/source/unit/quality/staleness observations and central temporal association without defaults | `docs/project-plan.md` phase 11 | #103 |
...
| `GATE-P00`-`GATE-P14` | Every phase exit gate | `docs/project-plan.md` phase sections | Owning phase issues |
