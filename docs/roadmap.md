...
`open(2)` flags for arm64) is delivered (PR #606); #602 (dual standalone smoke
403; in review on PR #607), #608 (single-agent production smoke fails on `main`
with a one-of-five rolling-combination lineage; claimed), #598 (signed upgrade
and rollback in the installer campaign), #597 (component-level image SBOM and
registry attestation), and #599 (signed release in the standalone preflight)
remain. #535 closes the initiative after them. One coordinator owns both queues and the shared
