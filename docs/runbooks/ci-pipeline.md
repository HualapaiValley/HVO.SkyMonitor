...
| **Integration Tests** | 462 SQLite, filesystem, SQL Server, Redis, MinIO, Mailpit, and host integration cases. Skipped only in classified reduced mode. |
...
The category audit requires every discovered case to belong to exactly one primary behavioral category. Current discovery is `Unit=1569`, `Integration=468`, `Manual=65`, `Soak=1`, `External=0`, and `Hardware=0`.
