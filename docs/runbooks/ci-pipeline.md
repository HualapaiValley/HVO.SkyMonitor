...
| **Integration Tests** | 518 SQLite, filesystem, SQL Server, Redis, MinIO, Mailpit, and host integration cases; the remaining six Integration-category cases run in Architecture & Publish. Skipped only in classified reduced mode. |
...
The category audit requires every discovered case to belong to exactly one primary behavioral category. Current discovery is `Unit=1725`, `Integration=524`, `Manual=73`, `Soak=1`, `External=0`, and `Hardware=1`.
