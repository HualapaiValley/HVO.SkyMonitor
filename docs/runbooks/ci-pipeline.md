...
| **Unit Tests** | 794 Unit cases with an intentionally invalid Docker endpoint and per-project TRX/Cobertura paths. |
| **Integration Tests** | 270 SQLite, filesystem, SQL Server, Redis, MinIO, Mailpit, and host integration cases. |
...
The category audit requires every discovered case to belong to exactly one primary behavioral category. Current discovery is `Unit=815`, `Integration=276`, `Manual=25`, `Soak=1`, `External=0`, and `Hardware=0`.
