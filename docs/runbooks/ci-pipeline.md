...
| **Unit Tests** | 1078 Unit cases with an intentionally invalid Docker endpoint and per-project TRX/Cobertura paths. Skipped only in classified reduced mode. |
| **Integration Tests** | 327 SQLite, filesystem, SQL Server, Redis, MinIO, Mailpit, and host integration cases. Skipped only in classified reduced mode. |
...
The category audit requires every discovered case to belong to exactly one primary behavioral category. Current discovery is `Unit=1078`, `Integration=333`, `Manual=37`, `Soak=1`, `External=0`, and `Hardware=0`.
