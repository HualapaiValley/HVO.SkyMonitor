...
| **Unit Tests** | 3239 Unit cases with an intentionally invalid Docker endpoint and per-project TRX/Cobertura paths. Runs only for the complete solution plan; component plans run the same per-project commands inside their selected lanes. |
...
The publication steps are best-effort because `Required CI` is the protected
code-quality gate. Missing configuration, an expired token, or a Gist outage
emits a workflow warning and summary but does not turn an otherwise healthy
main workflow red. Treat that warning as stale-badge evidence and repair it
before relying on the displayed percentages.
...
The category audit requires every discovered case to belong to exactly one primary behavioral category. Current discovery is `Unit=3239`, `Integration=656`, `Manual=102`, `Soak=1`, `External=0`, and `Hardware=1`.
