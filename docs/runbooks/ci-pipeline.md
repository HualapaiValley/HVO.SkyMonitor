...
| **Unit Tests** | 703 Unit cases with an intentionally invalid Docker endpoint and per-project TRX/Cobertura paths. |
| **Integration Tests** | 125 SQLite, filesystem, SQL Server, Redis, MinIO, Mailpit, and host integration cases. |
...
The category audit requires every discovered case to belong to exactly one primary behavioral category. Current discovery is `Unit=703`, `Integration=131`, `Manual=13`, `Soak=1`, `External=0`, and `Hardware=0`.
