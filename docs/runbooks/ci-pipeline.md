...
| **Unit Tests** | 3257 Unit cases with an intentionally invalid Docker endpoint and per-project TRX/Cobertura paths. Runs only for the complete solution plan; component plans run the same per-project commands inside their selected lanes. |
...
The category audit requires every discovered case to belong to exactly one primary behavioral category. Current discovery is `Unit=3257`, `Integration=656`, `Manual=102`, `Soak=1`, `External=0`, and `Hardware=1`.
