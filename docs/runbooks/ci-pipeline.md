...
| **Unit Tests** | 2256 Unit cases with an intentionally invalid Docker endpoint and per-project TRX/Cobertura paths. Skipped only in classified reduced mode. |
...
The category audit requires every discovered case to belong to exactly one primary behavioral category. Current discovery is `Unit=2256`, `Integration=580`, `Manual=80`, `Soak=1`, `External=0`, and `Hardware=1`.
