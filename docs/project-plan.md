...
| `SCHED-002` | Claimable work is ordered by starvation age first, then observatory priority, then weighted fair share `(active + served + 1) / weight` (unexpired leases plus attempts recorded within the fair-share window), breadth-first across observatories within equal keys, then availability, so a large old backlog from one observatory cannot starve another eligible observatory even when leases do not overlap. |
