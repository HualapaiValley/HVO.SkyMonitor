...
- **Sample APIs**: Versioned `/api/v1.0/sample/*` endpoints backed by `Result<T>` services for deterministic simulator responses.
...
```bash
cd /workspaces/HVO.SkyMonitor/src/HVO.SkyMonitor.CameraAgent.Simulator
dotnet run
```
...
```bash
dotnet ef database update --project src/HVO.SkyMonitor.CameraAgent.Simulator

dotnet ef migrations add <MigrationName> --project src/HVO.SkyMonitor.CameraAgent.Simulator
```
...
The existing `Dockerfile` still works for container builds. Run `docker build -t hvo-cameraagent-simulator -f src/HVO.SkyMonitor.CameraAgent.Simulator/Dockerfile .` from the repo root when you need an image.
