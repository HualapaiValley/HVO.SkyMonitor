...
internal sealed partial class CentralTransientValidationExecutor(
    ...
    ICelestialCatalogMetadataSource catalogMetadata,
    ...
    ILogger<CentralTransientValidationExecutor> logger) : ICentralTransientValidationExecutor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var masks = await CreateMasksAsync(lease, detectorSources, executionOptions, cancellationToken)
            .ConfigureAwait(false);
        if (masks is null)
        {
            return await CompleteNeedsReviewAsync(
    ...
    private async Task<TransientDetectorMask[]?> CreateMasksAsync(
        CentralDerivativeJobLease lease,
        IReadOnlyList<DetectorSource> sources,
        CentralTransientExecutionOptionsV1 options,
        CancellationToken cancellationToken)
    {
        if (lease.Inputs is null || sources.Count != Positions.Length || sources.Select(item =>
                item.Descriptor.Profiles.Mask).Distinct().Count() != 1)
        {
            return null;
        }
        ...
        var centralArtifactId = lease.Inputs[2].CentralArtifactId;
