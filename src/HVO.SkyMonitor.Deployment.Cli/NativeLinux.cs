...
internal static partial class NativeLinux
{
    ...
    [DefaultDllImportSearchPaths(DllImportSearchPath.System32)]
    private static partial int OpenAt(int directoryFileDescriptor, string path, int flags);

    [LibraryImport("libc", EntryPoint = "unlinkat", SetLastError = true, StringMarshalling = StringMarshalling.Utf8)]
    [DefaultDllImportSearchPaths(DllImportSearchPath.System32)]
    private static partial int UnlinkAt(int directoryFileDescriptor, string path, int flags);
    ...
    [DefaultDllImportSearchPaths(DllImportSearchPath.System32)]
    private static partial int FChmod(int fileDescriptor, uint mode);
    ...
    public static UnixNodeIdentity GetTargetNodeIdentity(string path)
    {
        const uint mask = StatxType | StatxMode | StatxLinkCount | StatxUid | StatxGid | StatxInode | StatxMountId;
        if (StatX(AtFileDescriptorCurrentWorkingDirectory, path, 0, mask, out var status) != 0 || (status.Mask & mask) != mask)
            throw new InstallerException($"Entry target '{path}' could not be authenticated.");
        return ToNodeIdentity(status);
    }
    ...
    public static SafeFileHandle OpenDirectoryNoFollow(string path)
    {
        ...
        return descriptor >= 0
            ? new SafeFileHandle(descriptor, ownsHandle: true)
            : throw new InstallerException($"Directory '{path}' could not be opened without following links.");
    }

    public static SafeFileHandle? TryOpenDirectoryAt(SafeFileHandle parent, string name)
    {
        var descriptor = OpenAt(
            parent.DangerousGetHandle().ToInt32(), name,
            OpenReadOnly | OpenDirectory | OpenNoFollow | OpenCloseOnExec | OpenNonBlocking);
        return descriptor >= 0 ? new SafeFileHandle(descriptor, ownsHandle: true) : null;
    }

    public static SafeFileHandle OpenNodeAt(SafeFileHandle parent, string name)
    {
        var descriptor = OpenAt(
            parent.DangerousGetHandle().ToInt32(), name,
        ...
        return descriptor >= 0
            ? new SafeFileHandle(descriptor, ownsHandle: true)
            : throw new InstallerException($"Entry '{name}' could not be opened without following links.");
    }
    ...
    public static UnixNodeIdentity GetOpenNodeIdentity(SafeFileHandle handle, string path)
    {
        const uint mask = StatxType | StatxMode | StatxLinkCount | StatxUid | StatxGid | StatxInode | StatxMountId;
        if (StatX(handle.DangerousGetHandle().ToInt32(), string.Empty, AtEmptyPath, mask, out var status) != 0 ||
            (status.Mask & mask) != mask)
    ...
    public static UnixNodeIdentity GetNodeIdentityAt(SafeFileHandle parent, string name)
    {
        const uint mask = StatxType | StatxMode | StatxLinkCount | StatxUid | StatxGid | StatxInode | StatxMountId;
        if (StatX(parent.DangerousGetHandle().ToInt32(), name, AtSymbolicLinkNoFollow, mask, out var status) != 0 ||
            (status.Mask & mask) != mask)
        {
    ...
    public static void UnlinkNodeAt(SafeFileHandle parent, string name, bool directory)
    {
        if (UnlinkAt(parent.DangerousGetHandle().ToInt32(), name, directory ? AtRemoveDirectory : 0) != 0)
        {
            throw new InstallerException($"Entry '{name}' could not be removed through its authenticated parent.");
        }
    }

    public static void RenameNodeAt(SafeFileHandle parent, string oldName, string newName)
    {
        var descriptor = parent.DangerousGetHandle().ToInt32();
        if (RenameAt2(descriptor, oldName, descriptor, newName, RenameNoReplace) != 0)
        {
            throw new InstallerException($"Entry '{oldName}' could not be renamed through its authenticated parent.");
        }
    }

    public static void MakeOpenDirectoryOwnerWritable(SafeFileHandle handle)
    {
        if (FChmod(handle.DangerousGetHandle().ToInt32(), 0x1C0) != 0)
        {
            throw new InstallerException("Authenticated deletion could not make a directory owner-writable.");
        }
    }
    ...
    private static UnixNodeIdentity ToNodeIdentity(LinuxFileStatus status)
        => new(
            status.Uid,
            status.Gid,
            (UnixFileMode)(status.Mode & 0x0FFF),
...
internal sealed record UnixNodeIdentity(
    uint Uid,
    uint Gid,
    UnixFileMode Mode,
    uint Type,
